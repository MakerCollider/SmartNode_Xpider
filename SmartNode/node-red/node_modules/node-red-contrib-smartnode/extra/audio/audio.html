...
<script type="text/javascript">
    ...
    RED.nodes.registerType('Audio', {
        ...
        icon: "light.png",
        label: function() {
            return this.name || this._("Audio.label.palette");
        },
        paletteLabel: function () {
            return this._("Audio.label.palette");
        },
        oneditprepare: function() {
            ...
            var upload_url = web_url+'/index.html?type='+fileType;
            loadFrame(upload_url,web_url,fileType);
            ...
            $("#node-input-add-file").click(function() {
                addSingleFileSelect(web_url,fileType);
            }); 
    ...
    function loadFrame(upload_url,_url,_fileType){
        $('#frmUpload').attr('src',upload_url);
        getFilesList(_url,_fileType);
    }

    function getFilesList(_url,_fileType){
        $.ajax({
            url: _url+'/getfiles?type='+_fileType,
            type: 'GET',
            dataType: "json",
            cache: false,
            success: function(data){
                if (200 === data.code) {
                    files = data.msg;
                    for (var i in rules){
                        generateMultiFileSelect(parseInt(i)+1,rules[i],_url,_fileType);
                    }
                } 
            },
            error: function(){}
    ...
    function addSingleFileSelect(_url,_fileType){
        $.ajax({
            url: _url+'/getfiles?type='+_fileType,
            type: 'GET',
            dataType: "json",
            ...
            success: function(data){
                if (200 === data.code) {
                    ...
                    generateSingleFileSelect(parseInt(j)+1,files,_url,_fileType); 
                } 
            },
            error: function(){}
        });
    ...
    function appendNewOptions(selectField,_url,_fileType){
        $.ajax({
            url: _url+'/getfiles?type='+_fileType,
            type: 'GET',
            dataType: "json",
    ...
    function generateMultiFileSelect(i,rule,_url,_fileType) {
        ...
        selectField.click(function() {
            appendNewOptions(this,_url,_fileType);
        });
    }


    function generateSingleFileSelect(i,_files,_url,_fileType) {
        var container = $('<li/>',{style:"background: #fff; margin:0; padding:8px 0px; border-bottom: 1px solid #ccc;"});
        var row = $('<div/>').appendTo(container);
        $('<i style="color: #eee; cursor: move;" class="node-input-rule-handle fa fa-bars"></i>').appendTo(row);
